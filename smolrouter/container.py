...
class SmolRouterContainer:
    ...
    async def _perform_startup_health_checks(self):
                ...
                if is_healthy:
                    ...
                    try:
                        ...
                        if models:
                            ...
                            await self._cache.cache_models(provider_id, models, self.config.cache_ttl)
        ...
        if healthy_count == 0:
            ...
            logger.warning(
                f"⚠ {total_count - healthy_count} provider(s) are unhealthy. Check configuration and connectivity."
            )
            ...
            try:
                ...
                if is_healthy:
                    ...
                    if not await self._cache.is_cache_valid(provider.get_provider_id()):
                        try:
                            models = await provider.discover_models()
                            await self._cache.cache_models(provider.get_provider_id(), models, self.config.cache_ttl)
                            logger.debug(
                                f"Refreshed cache for provider {provider.get_provider_id()}: {len(models)} models"
                            )
                        except Exception as e:
    ...
    def _create_cache(self) -> IModelCache:
        ...
        return InMemoryModelCache(
            default_ttl=self.config.cache_ttl, cleanup_interval=self.config.cache_cleanup_interval
        )
