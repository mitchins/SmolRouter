...
class SmartModelStrategy(IModelStrategy):
    ...
    async def resolve_model_request(
            ...
            for model in available_models:
                if model.provider_id == provider_id and model.matches_request(model_name):
                    ...
                    return ModelResolution(
                        model=model, resolved_from=requested_model, resolution_path=resolution_path
                    ).model
        ...
        exact_matches = [model for model in available_models if model.matches_request(requested_model)]
        ...
        if exact_matches:
            ...
            return ModelResolution(model=selected, resolved_from=requested_model, resolution_path=resolution_path).model

        # Step 4: Find partial matches
        partial_matches = []
        for model in available_models:
            if requested_model.lower() in model.name.lower() or any(
                requested_model.lower() in alias.lower() for alias in model.aliases
            ):
        ...
        if partial_matches:
            ...
            return ModelResolution(
                model=selected, resolved_from=requested_model, fallback_used=True, resolution_path=resolution_path
            ).model
