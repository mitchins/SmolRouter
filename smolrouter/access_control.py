...
class IPBasedAccessControl(IAccessControl):
    ...
    def _compile_rules(self):
        ...
        for ip_pattern, rule_config in self.rules.items():
            ...
            rule["allowed_model_regexes"] = [
                re.compile(p[1:-1]) if p.startswith("/") and p.endswith("/") else None for p in rule["allowed_models"]
            ]

            rule["denied_model_regexes"] = [
                re.compile(p[1:-1]) if p.startswith("/") and p.endswith("/") else None for p in rule["denied_models"]
            ]
    ...
    async def filter_models(self, models: List[ModelInfo], client: ClientContext) -> List[ModelInfo]:
        ...
        if rule["allow_all"]:
            ...
            logger.debug(
                f"Allowed {len(filtered_models)}/{len(models)} models for IP {client.ip} (allow_all with deny list)"
            )
